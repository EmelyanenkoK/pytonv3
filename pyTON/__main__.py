...
async def main(loop):
    ...
    parser.add_argument('--keystore', '-k', default=None, type=str)
    parser.add_argument('--cdll', '-D', default=None, type=str)
    ...
    def wrap_result(func, high_timeout = False):
      ...
      async def wrapper(*args, **kwargs):
        try:
          ...
          return web.json_response( { "ok": False, "error": "Request timeout" }, headers=headers)
        except Exception as e:
          try:
            traceback.print_exc()
    ...
    @routes.get('/tryLocateTx')
    @json_rpc('tryLocateTx', 'get')
    @wrap_result
    async def getTryLocateTx(request):
      source = request.query['source']
      destination = request.query['destination']
      created_lt = int(request.query['created_lt'])
      return await tonlib.tryLocateTxByIncomingMessage(source, destination, created_lt)

    @routes.get('/tryLocateResultTx')
    @json_rpc('tryLocateResultTx', 'get')
    @wrap_result
    async def getTryLocateTx(request):
      source = request.query['source']
      destination = request.query['destination']
    ...
    @routes.get('/tryLocateSourceTx')
    @json_rpc('tryLocateSourceTx', 'get')
    @wrap_result
    async def getTryLocateTx(request):
      source = request.query['source']
      destination = request.query['destination']
